...
def replace_spaces_in_rule_field(input_file, output_file=None):
    """
    Read a pipe-delimited CSV and convert the second field (Rule column) to uppercase.
            ...
            with open(output_file, "w", encoding="utf-8", newline="") as outfile:
                ...
                for row in reader:
                    if len(row) >= 2:  # Ensure row has at least 2 columns
                        original_rule = row[1]
                        # Convert to uppercase in the second field only
                        row[1] = row[1].upper()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        input_filename = "values_emotions_dataset_joy_modified.csv"  # Change this to your CSV filename
